...
def contains_keyword(ArxivType, word_to_search) -> List[str]:
    ...
    for articles in ArxivType:
        if search_word in articles:
            return_list.append(articles)
        return_list.sort
    return return_list
...
def read_arxiv_file(TextIO) -> ArxivType:
    """Return the data in the file TextIO in ArxivType dictionary format.
    """
...
def get_coauthors (ArxivType, NameType) -> List[NameType]:
    """Return a list of coauthors of the author NameType in the metadata 
    ArxivType
    
    """
    

def get_most_published_authors(ArxivType) -> List[NameType]:
    """Return a list of authors who published the most articles in the ArxivType
    metadata
    
    """
    
def suggest_collaborators(ArxivType, NameType) -> List[NameType]:\
    """Return a list of authors with whom the author NameType is encouraged to 
    collaborate with, ie. the list of authors who are coauthors of this author's
    coauthors 
